...
The verified harnesses are `claude`, `codex`, `codex-personal`, `codex-martos`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `muse` for crewmates and scouts only; `codex-martos` is also crewmate/scout-only and never a primary or secondmate supervision harness; never dispatch on an unverified adapter.
