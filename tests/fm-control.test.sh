...
VERIFIED_HARNESSES="claude codex codex-personal codex-martos opencode pi pi-signed grok kimi cursor muse"
...
verified_adapter_contract() {  # <harness> -> exit command, interrupt key, repeat, clear key
  case "$1" in
    ...
    codex-personal) printf '/quit\tEscape\t1\t\n' ;;
    codex-martos) printf '/quit\tEscape\t1\t\n' ;;
...
test_harness_family_resolution() {
  ...
  for pair in claude:claude claude-latest:claude codex:codex codex-cli:codex \
      codex-personal:codex-personal codex-martos:codex-martos \
...
test_harness_kind_capability() {
  ...
  for harness in claude codex codex-personal opencode pi pi-signed grok kimi; do
    fm_control_harness_supports_kind "$harness" secondmate \
      || fail "$harness should be able to run a secondmate"
  done
  fm_control_harness_supports_kind codex-martos secondmate \
    && fail "codex-martos is a crewmate/scout profile and must not claim a secondmate"
